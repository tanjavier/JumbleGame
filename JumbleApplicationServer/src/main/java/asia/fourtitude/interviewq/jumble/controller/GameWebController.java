...
public class GameWebController {
    ...
    public String doGetNew(@ModelAttribute(name = "board") GameBoard board) {
        ...
        board.setWord(""); // Initialize empty word for guessing
        return "game/board";
    }
    ...
    public String doPostPlay(
        ...
        String guessedWord = board.getWord() != null ? board.getWord().trim().toLowerCase() : "";
            
        // Clear the input field after processing
        board.setWord("");

        // If no word was entered, just refresh the board
        if (guessedWord.isEmpty()) {
            return "game/board";
        }

        // Try to update the game state with the guessed word
        boolean isCorrect = board.getState().updateGuessWord(guessedWord);
        
        if (!isCorrect) {
            // If guess is incorrect, add error message to the word field
